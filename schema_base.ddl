...
ALTER TABLE salaries ADD CONSTRAINT salaries_ck_1 CHECK ( salary > 0 );

ALTER TABLE salaries ADD CONSTRAINT salaries_pk PRIMARY KEY ( jobid,
shopid );
...
CREATE TABLE stockitems (
    stocktid          INTEGER NOT NULL,
    productid         INTEGER NOT NULL,
    count             INTEGER,
    manufacturedate   DATE
);

ALTER TABLE stockitems ADD CONSTRAINT stockitems_pk PRIMARY KEY ( stocktid,
productid );
...
ALTER TABLE salaries
    ADD CONSTRAINT salaries_jobs_fk FOREIGN KEY ( jobid )
        REFERENCES jobs ( jobid );

ALTER TABLE salaries
    ADD CONSTRAINT salaries_shops_fk FOREIGN KEY ( shopid )
        REFERENCES shops ( shopid );
...
ALTER TABLE stockitems
    ADD CONSTRAINT stockitems_stockchanges_fk FOREIGN KEY ( stocktid )
...
CREATE SEQUENCE stockitems_stocktid_seq START WITH 1 NOCACHE ORDER;

CREATE OR REPLACE TRIGGER stockitems_stocktid_trg BEFORE
    INSERT ON stockitems
    FOR EACH ROW
    WHEN ( new.stocktid IS NULL )
BEGIN
    :new.stocktid := stockitems_stocktid_seq.nextval;
END;
/
